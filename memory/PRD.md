...
- ✅ **Cohort Retention Analytics** — new `GET /api/admin/analytics/cohorts?weeks=8` (admin-only) buckets users by ISO signup-week (last N weeks, capped 1..26) and returns `{signups, activated, shared, activated_pct, shared_pct}` per week + all-time totals. Activation = created ≥1 project; Shared = published a share link. Robust to string OR datetime `created_at` values, and to timezone-less legacy timestamps. Admin Panel → Analytics tab now renders a new "Signup-week cohorts / Retention & activation by cohort" card with top-line totals, 8-row table, and a stacked bar chart (signups vs activated vs shared) via recharts. Test-ids `cohorts-section`, `cohorts-table-body`, `cohort-row-{iso_date}`, `cohorts-total-activated-pct`, `cohorts-total-shared-pct`. E2E verified: 143 signups this week, 10.5% activation, 0.7% share rate; chart + table both render correctly.
