...
- ✅ **Pricing Page Rebuilt** — new tile grid pulled from `/api/pricing/config` (falls back gracefully if API is offline), popular-plan highlighting, live credit calculator, Sora premium-video tooltip (200 credits/sec), waitlist CTA. Rupee formatting via `toLocaleString("en-IN")`. Test-ids `pricing-plans-grid`, `plan-{free|creator|business|agency}`, `topup-strip`, `sora-tooltip`, `picked-credit-cost`.
- ✅ **Zero-loss Data Migration** — one-shot idempotent migration (marker: `system_migrations.credits_17x_v1`) multiplied all 173 existing user balances by 17× (11,213 → 190,621 total credits) to preserve their pre-repricing value. Admin's 10,050 → 170,850. No user got punished by the repricing.
- ✅ **LowCreditNudge upgraded** — threshold now matches backend (200 credits), copy updated to explicitly show current balance + dual CTA (Top up + Invite a friend for 50 free credits).
- ✅ **Wizard credit map + Landing FAQ** synced with the new pricing so users see accurate costs everywhere.

## Phase 15 (2026-02) — Pipeline Thumbnail + UX Modernization + Critical Bug Fixes
- ✅ **Video Thumbnail Generation stage** — added a new **"generating thumbnail"** stage (progress=95%) between compose and done. Uses `ffmpeg -ss 1.0 -frames:v 1 -vf scale=…1280x720 -q:v 3` to extract a 720p JPEG poster from the composed video at t=1s (or the middle for short videos), saved to `storage/thumbnails/{project_id}.jpg` and exposed at `/api/media/thumbnails/*`. Stored on the project as `thumbnail_url`. Non-fatal on failure: the project still ships and the frontend falls back to `scenes[0].image_url`. Frontend now uses the real thumbnail everywhere: Dashboard card poster, download-thumbnail button (JPG), quick-preview `<video poster>`, ProjectView `<video poster>`, PublicVideo `<video poster>`. Public share endpoint also returns `thumbnail_url`.
- ✅ **Approval Flow UI Modernization** — completely redesigned all 3 approval gates (Script / Visuals / Voice) with a shared 4-step `ApprovalStepIndicator` (Script → Visuals → Voice → Final) as pill-chips with checked/current/pending states. Replaced flat brand-600 headers with a premium gradient (brand-600 → indigo-700, soft shadow), font-black tracking-tighter heading, and shadow-lifted primary CTAs.
- ✅ **Reliable Download Helper** — new `downloadFile()` uses `fetch` + `URL.createObjectURL` to force filenames. Fixes blank-filename downloads across Dashboard, ProjectView, and quick preview.
- 🐛 **Fixed Recurring Caption Tofu Boxes** — installed `fonts-noto` + `fonts-noto-cjk`, added boot self-heal + `POST /api/admin/repair/fonts`.
- 🐛 **Fixed "Failed to start" Video Generation (Critical)** — legacy `share_slug` null value broke sparse index; unset the null + rebuilt with `partialFilterExpression`.
- ✅ **Enhance-topic length limit raised** 500 → 2000 chars.
- ✅ **Onboarding step 3 quick-share** — inline "Publish share link" button.
- ✅ **Onboarding cosmetic fix** — removed strikethrough on completed steps.
