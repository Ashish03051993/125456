...
- ✅ **Kadenza rebrand sweep** — wordmark, browser title, OG/Twitter meta, footers, copy and social-share text updated across 15 files (Layout, Login, Signup, ForgotPassword, ResetPassword, NotFound, Landing, Pricing, Legal, PublicVideo, ShortLinkRedirect, AdminPanel, ProjectView, DemoVideoSection, WaitlistForm, index.html). Backend FastAPI title also updated. Grep verified: zero residual `AI Video Studio` / `AI Video<span>Studio</span>` refs in user-facing files.
